...
\newenvironment{NewUnicodeEncoding}[1]{%
  \let\OldUnicodeEncodingName\UnicodeEncodingName
  ...
  \let\UnicodeEncodingName\OldUnicodeEncodingName
}

\begin{NewUnicodeEncoding}{TUX}
  \DeclareUnicodeCompositeCommand{\c}{G}{\fbox{cg}}
\end{NewUnicodeEncoding}

\begin{document}

hello firefly

\def\TEST#1{\texttt{\detokenize{#1}}: `{\fontencoding{T1}\selectfont#1}' / `{\fontencoding{TU}\selectfont#1}'\par}
\parindent=0pt\relax
\raggedright
...
\TEST{\'e}
\TEST{\c G}
{TUX: \fontencoding{TUX}\selectfont {\c G}}


\" \i\ = \char"00EF\relax \i\char "0308\relax\ $\neq$ 
